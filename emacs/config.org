...
;; I have a directory in my Dropbox used for "quick", e.g. notes for a impromptu meeting, a quick project, etc.
;; The following methods are useful for interacting with this directory.
(defvar tor/dropbox-quick-directory "~/Dropbox/quick/"
    "Directory in Dropbox used for quick notes, projects, etc.")

(defun tor/dropbox-quick-make-dir (name)
  "Create a directory in ~/Dropbox/quick/ with name constructed from current date, time and NAME."
  (interactive "sName: ")
  (let* ((timestamp (format-time-string "%Y-%m-%d--%H-%M"))
         (dir (concat tor/dropbox-quick-directory timestamp "--" name)))
    (make-directory dir)
    (find-file dir)))

(defun tor/dropbox-quick-go-to (name)
  "Go to a directory in ~/Dropbox/quick/ with NAME."
  ;; Use autocomplete.
  ;; TODO: Sort according to last modified.
  (interactive (list (completing-read "Name: " (directory-files tor/dropbox-quick-directory nil "^[^.]"))))
  (find-file (concat tor/dropbox-quick-directory name)))

(defun tor/bookmark-set ()
  "Set a bookmark for the current file using the current line number."
  (interactive)
  (bookmark-set (concat (buffer-file-name) ":" (number-to-string (line-number-at-pos)))))
#+end_src
...
#+name: basics--dropbox
#+begin_src emacs-lisp
;; TODO: Implement a way to conditionally run code depnding on whether dropbox is available.
(defun dropbox-available-p ()
  "Return t if Dropbox is available, nil otherwise."
  (file-exists-p (expand-file-name "~/Dropbox")))

(defmacro with-dropbox (&rest body)
  "Execute BODY only if Dropbox is available."
  `(when (dropbox-available-p)
     ,@body))
#+end_src
...
- MacOS:
  1. Install =cmake= using =brew=.
  2. Install =libvterm= using =brew=.
  3. Then start Emacs and run =vterm=.
...
(use-package copilot
  ...
  :custom
  ;; Disable warning messages regarding indentation offset not being inferred, etc.
...
(use-package org
  ...
  ;; Adds some `private/org-capture-templates' templates I don't want on my Github.
  ;; But if it's not defined, we just define it as an empty list.
  (unless (boundp 'private/org-capture-templates)
    (setq private/org-capture-templates '()))
...
;; Some functionality to copy the previewed LaTeX fragment to the clipboard.
(defun tor/yank-image-to-clipboard--xclip (image-path)
  "Copy the image at IMAGE-PATH to the clipboard using xclip."
  (if (file-exists-p image-path)
      ;; NOTE: The `-quiet' argument is neceessary though I don't quite understand why.
      (start-process "xclip-proc" nil "xclip" "-quiet" "-selection" "clipboard" "-t" "image/png" "-i" (file-truename image-path))
    (message "File %s does not exist" image-path)))

(defun tor/yank-image-to-clipboard--macos (image-path)
  "Copy the image at IMAGE-PATH to the clipboard using xclip."
  (if (file-exists-p image-path)
      ;; NOTE: The `-quiet' argument is neceessary though I don't quite understand why.
      (start-process "xclip-proc" nil "impbcopy" (file-truename image-path))
    (message "File %s does not exist" image-path)))

(defun tor/yank-image-to-clipboard (image-path)
  (message "%s" image-path)
  (let ((tor/yank-image-to-clipboard--inner (if (eq system-type 'darwin)
                                                #'tor/yank-image-to-clipboard--macos
                                              #'tor/yank-image-to-clipboard--xclip)))
    (cond
     ;; If the path points to a PDF, convert it to a PNG and copy it to the clipboard.
     ((string-match-p "\\.pdf\\'" image-path)
      (let ((png-path (concat (file-name-sans-extension image-path) ".png")))
        (shell-command (format "convert -density 300 %s %s %s" image-path (if arg "-flatten -background white" "") png-path))
        (funcall tor/yank-image-to-clipboard--inner png-path)))
     ;; If it points to an SVG file, do the same.
     ((string-match-p "\\.svg\\'" image-path)
      (let ((png-path (concat (file-name-sans-extension image-path) ".png")))
        (message "%s" png-path)
        (shell-command (format "convert -density 300 %s %s %s" image-path (if arg "-flatten -background white" "") png-path))
        (funcall tor/yank-image-to-clipboard--inner png-path)))
     (t (funcall tor/yank-image-to-clipboard--inner image-path)))))

(defun tor/yank-image-in-buffer-to-clipboard (arg)
  "Copy the image in the current buffer to the clipboard.

If the prefix argument is given, a white background is added to the image."
  (interactive "P")
  (let ((image-path (buffer-file-name)))
    (tor/yank-image-to-clipboard image-path)))

(defun tor/org-latex-preview-yank (&optional arg)
  "Yank the previewed LaTeX fragment to the clipboard.

If the prefix argument is given, just print the path of the image."
  ;; Adopted from `org-format-latex'.
  (interactive "P")
  (let* ((forbuffer 'forbuffer)
		 ...
		 (abspath (format "%s_%s.%s" absprefix hash imagetype)))
    (message "%s vs. %s" imagetype (plist-get processing-info :image-output-type))

    ;; Copy the image at the `abspath' to the clipboard.
    (if (not arg)
        (progn
          ;; Yank by default.
          (tor/yank-image-to-clipboard abspath)
          (message "Image %s copied to clipboard" abspath))
      ;; Otherwise, just print the path of the image.
      (message "Image path: %s" abspath)
      )
    ))
#+end_src
...
(defun org-display-inline-image-in-results ()
  ...
  (interactive)
  (save-excursion
    (let ((target-point (org-babel-where-is-src-block-result)))
        (when target-point
...
(defun my/citar-org-format-note-default (key entry)
  ...
  (let* ((template (citar--get-template 'note))
    ...
    (with-current-buffer buffer
      ...
      (insert "#+SETUPFILE: ../setup-level-1.org\n")
      (insert "#+TITLE: ")
...
(defun my/citar-set-keywords (citekey keywords-new)
  "Set the keywords of the CITEKEY to KEYWORDS-NEW."
  (save-window-excursion
    (citar-open-entry citekey)
    ;; TODO: Maybe don't rely on `org-ref' for this?
    (org-ref-set-bibtex-keywords keywords-new t)))

(defun my/citar-edit-keywords (citekey)
  "Edit the keywords of the CITEKEY."
  (interactive (list (citar-select-ref)))
  (let* ((entry (citar-get-entry citekey))
         (keywords (cdr (assoc "keywords" entry)))
         (new-keywords (read-string "Keywords: " keywords)))
    (my/citar-set-keywords citekey new-keywords)))

;; TODO: Make it work for multiple keys?
(defun my/citar-copy-keys (citekey)
  "Copy CITEKEYS."
  (interactive (list (citar-select-ref)))
  (kill-new citekey)
  (message "Copied key %s." citekey))
...
(use-package citar
  ...
  (citar-notes-paths '("~/org-blog/papers/"))
  (citar-note-format-function 'my/citar-org-format-note-default)
  ...
  (setq citar-map (let ((map (make-sparse-keymap)))
                    ...
                    (define-key map (kbd "K") #'my/citar-copy-keys)
                    ...
                    (define-key map (kbd "C-k") #'my/citar-edit-keywords)
                    map))
...
** Internship config
#+name: el-intern-init
#+begin_src emacs-lisp :comments noweb :tangle init-intern.el :noweb yes
<<pkg-management>>
<<basics>>
<<os-specifics>>
<<ui>>
